...
from layers import GCN, RiemannianGNN
...
class HierarchicalAggrLayer(nn.Module):
    def __init__(self, args, manifold, in_rels, out_rels):
        super().__init__()
        self.in_rels = in_rels
        self.out_rels = out_rels
        self.is_attn = args.is_attn

        self.hgcns = nn.ModuleList([RiemannianGNN(args, manifold) for _ in range(in_rels)])
        self.alphas = nn.Parameter(torch.FloatTensor(out_rels, in_rels))

        # For attention
        if self.is_attn:
            self.V = nn.ModuleList([nn.Linear(args.hid_dim, args.hid_dim, bias=False) for _ in range(in_rels)])
            self.Y = nn.ModuleList([nn.Linear(args.hid_dim, 1) for _ in range(in_rels)])
    ...
    def forward(self, z_in, adjs_in, sparse=False):
        ...
        for i in range(self.in_rels):
            z = self.hgcns[i](z_in, adjs_in[i], sparse)
            z_out_all.append(z)
